...
### 5.1 下载工具增强 (Skip-if-exists)
- **脚本**: `download_3gpp_docs.py`
- **特性**: 增加了对最终生成文件的检查机制。
- **逻辑**: 在发起下载请求前，脚本会根据当前配置预测最终输出路径（`.json` 或 `.docx`）。如果文件已存在于目标目录，将跳过该文件的下载与解压，极大节省了重复运行的时间和带宽。

### 5.2 爬虫工具增强 (Resume/Incremental Update)
- **脚本**: `3gpp_spec_scope_spider.py`
- **特性**: 支持断点续传与增量更新。
- **逻辑**: 脚本启动时会自动读取现有的汇总报告。如果发现某个 Spec 已在报告中且状态为成功，则会从本次爬取列表中自动过滤掉该 Spec。处理完成后，新结果会与旧结果进行智能合并，方便在网络中断或需要补充更新时使用。
