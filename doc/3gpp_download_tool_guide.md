...
## 5. 常见问题 (FAQ)
1. **找不到协议？**
   *   请确保 `--rel` 和 `--series` 匹配。例如 38 系列通常在 Rel-15 及之后才有。
   *   `--specs` 匹配不区分点号，`38.331` 和 `38331` 效果相同。
2. **下载速度慢？**
   *   3GPP FTP 服务器位于欧洲，国内访问可能较慢，建议增加 `-t` 参数或在网络环境较好的地方运行。
3. **JSON 转换失败？**
   *   脚本依赖 Python 标准库。如果 docx 格式极端复杂（如大量嵌套表格），转换器会尝试提取纯文本内容。

## 6. AI 使用提示 (Prompt Helper)
如果你是 AI Agent，可以参考以下逻辑使用此工具：
1. **查找范围**：当用户询问某项技术（如 "RedCap"）在 Rel-18 中的定义时。
2. **第一步**：执行 `python3 download_3gpp_docs.py -r 18 -s 38 --specs 38.331 --json`。
3. **第二步**：读取生成的 `.json` 文件。
4. **第三步**：在 `content` 字段中搜索关键词 "RedCap"。
5. **第四步**：根据 JSON 中的 `title`（章节名）为用户提供带出处的准确回答。
